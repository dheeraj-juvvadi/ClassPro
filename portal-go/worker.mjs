...
export function createWorker({ token, createSession, maxSessions = 4, maxConcurrent = 2, timeoutMs = 85000, closeTimeoutMs = 3000, onCloseTimeout = () => {} }) {
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      entry.busy = true;
      acquired = true;
      const operation = run(action, payload, entry, fresh);
...
async function run(action, payload, entry, fresh) {
  ...
  const result = await portal.login(payload.account, payload.password, payload.answer);
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  process.env.PORTAL_LOGIN_DIAGNOSTICS = '0';
